...
namespace GUI_PolyCafe
{
    public partial class frmLoaiSanPham : Form
    {
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvDanhSachLoaiSP.DataSource = null;
                dgvDanhSachLoaiSP.DataSource = bus.GetLoaiSanPhamList();
                return;
            }

            var ketQua = bus.TimKiemLoaiSanPham(tuKhoa);

            if (ketQua.Count > 0)
            {
                dgvDanhSachLoaiSP.DataSource = null;
                dgvDanhSachLoaiSP.DataSource = ketQua;
            }
            else
            {
                MessageBox.Show("Không tìm thấy loại sản phẩm phù hợp.");
                dgvDanhSachLoaiSP.DataSource = null;
            }
        }
    }
