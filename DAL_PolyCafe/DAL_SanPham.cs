...
namespace DAL_PolyCafe
{
    public class DAL_SanPham
    {
        ...
        public decimal getDonGiaByMaSanPham(string maSanPham)
        {
            decimal donGia = 0;
            try
            {
                string sql = "SELECT DonGia FROM SanPham WHERE MaSanPham = @0";
                List<object> parameters = new List<object> { maSanPham };
